...
function publication__build_citation( $id, $build_mode = MDB_BUILD_STRING )
{
    ...
    if( ! empty( $data['authors'] ) ) :
        ...
        $output[0] .= __( 'without author', 'mdb_theme_core' );
    endif;
    ...
    if( ! empty( $data['title'] ) or ! empty( $data['subtitle'] ) ) :
        ...
        $title = __( 'without a title', 'mdb_theme_core' );
    endif;
    ...
    if( ( 'ARTICLE' === $type[$data['type']] ) ) :
        ...
        if( 'CHAPTER' === $type[$data['type']] ) :
            ...
            if( ! empty( $data['editors'] ) ) :
                ...
                $editors .= __( 'without publisher', 'mdb_theme_core' );
            endif;

            $output[1] .= sprintf(
                __( ' In: %1$s [Pub.]: %2$s', 'mdb_theme_core' ),
                $editors,
        ...
        if( ! empty( $data['edition'] ) ) :
            ...
            if( ! empty( $data['issue'] ) ) :
                $output[1] .= sprintf(
                    __( ' Volume %1$s of the series "%2$s".', 'mdb_theme_core' ),
                    $data['issue'],
                    $data['edition'],
                );
            else :
                $output[1] .= sprintf(
                    __( ' Of the series "%1$s".', 'mdb_theme_core' ),
                    $data['edition'],
                );
            endif;
        endif;
